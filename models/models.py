...
#
# class WN_GCN_layer_reverse(nn.Module):
#     '''
#     Implementation of WN-GCN
#     '''
#     def __init__(self, args, pointconv_cfg):
#         super(WN_GCN_layer_reverse, self).__init__()
...
class WN_GCN(nn.Module):
    ...
    def make_layer(self, args, cfgs, reverse):
        ...
        return torch.nn.Sequential(*layer)
#
# class WN_GCN_reverse(nn.Module):
#     '''
#     Weighted nearest-Neighbor Graph Convolutional Network Module
#     '''
#
#     def __init__(self, args, cfgs, cfgs_cls, cosface=True):
#
#         super(WN_GCN_reverse, self).__init__()
#         self.encoder = self.make_layer(args, cfgs)
#         self.GAP = torch.nn.AdaptiveAvgPool2d((1, 1))
#         if cosface:
#             self.classifier = CosFaceClassifier(cfgs_cls)
#         else:
...
#         return x, equi_feat, inv_feat
#
#     def make_layer(self, args, cfgs):
#         layer=[]
#         for v in cfgs:
#             if isinstance(v, list):
#                 layer += [WN_GCN_layer_reverse(args, v)]
#         return torch.nn.Sequential(*layer)
